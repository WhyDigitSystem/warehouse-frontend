...
const fetchPartList = async () => {
  try {
    // Using axios directly as in your working example
    ...
    if (response.data?.status === true) {
      const materialVO = response.data?.paramObjectsMap?.materialVO || [];
      
      if (materialVO.length > 0) {
        const parts = materialVO
        ...
        console.warn("No parts found in API response");
        setPartList([{ partno: "ALL", partDesc: "All Parts", id: null }]);
      }
    } else {
      console.error("API returned false status");
      addToast("error", "Failed to load parts");
      setPartList([{ partno: "ALL", partDesc: "All Parts", id: null }]);
    }
  } catch (error) {
    console.error("Error fetching part list:", error);
    addToast("error", "Failed to load parts");
    // Set default with "ALL" option even on error
    setPartList([{ partno: "ALL", partDesc: "All Parts", id: null }]);
  }
};
...
// Add a useEffect to log partList when it changes
useEffect(() => {
  console.log("Current partList:", partList);
  console.log("Filtered parts:", filteredParts);
}, [partList, filteredParts]);
...
const fetchStockLedger = async () => {
  ...
  try {
    ...
    // Use direct axios call as a quick fix
    const response = await axios.get(`${API_URL}/api/Reports/getStockLedger`, {
      params: {
        branchCode: loginBranchCode,
        client: loginClient,
        customer: loginCustomer,
        orgId: orgId,
        partNo: partNo === "ALL" ? "" : partNo,
    ...
    if (apiResponse && (apiResponse.status === true || apiResponse.statusFlag === "Ok")) {
      const stockDetails = apiResponse.paramObjectsMap?.stockDetails || [];
      console.log("Stock details found:", stockDetails);
      ...
      setList([]);
      addToast("error", apiResponse.paramObjectsMap?.message || "No data found");
    }
  } catch (error) {
    console.error("Error:", error);
    setList([]);
    addToast("error", "Failed to fetch stock ledger data");
  } finally {
    setLoading(false);
    setRefreshing(false);
  }
  ...
  const handleClearFilters = () => {
    setStartDate(dayjs().format("DD-MM-YYYY"));
  ...
  const handleExcelDownload = () => {
    ...
    try {
      ...
      // Calculate totals
      const totalQty = list.reduce((sum, item) => sum + parseFloat(item.cQty || 0), 0);
      ...
      const fileName = `Stock_Ledger_Summary_${dayjs().format("YYYY-MM-DD_HH-mm")}.xlsx`;
      ...
      addToast("success", "Excel report with summary downloaded");
    } catch (error) {
  ...
  const filtered = list.filter((item) => {
    ...
    return (
      ...
      (item.sourceScreen && item.sourceScreen.toLowerCase().includes(searchLower))
    );
  });

  // Calculate total quantity
  const totalQty = filtered.reduce((sum, item) => sum + parseFloat(item.cQty || 0), 0);
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="flex-1 min-w-[180px]">
            ...
            <div className="flex items-center">
              ...
              <input
                type="date"
                value={startDate.includes("-") ? dayjs(startDate, "DD-MM-YYYY").format("YYYY-MM-DD") : startDate}
                onChange={(e) => {
          ...
          <div className="flex-1 min-w-[180px]">
            ...
            <div className="flex items-center">
              ...
              <input
                type="date"
                value={endDate.includes("-") ? dayjs(endDate, "DD-MM-YYYY").format("YYYY-MM-DD") : endDate}
                onChange={(e) => {
                  const newDate = dayjs(e.target.value).format("DD-MM-YYYY");
                  setEndDate(newDate);
                }}
    ...
    <div className="flex flex-wrap items-center gap-3 text-xs">
      ...
      {showResultsSearch && list.length > 0 && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 flex items-center gap-2 mb-4 shadow-sm">
          ...
          <input
            type="text"
            placeholder="Search by Part No, Description, Ref No, or Source Screen..."
      ...
      {!loading && list.length === 0 && partNo && (
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2">
            No Stock Ledger Data Available
          </h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            No stock ledger found for the selected parameters.
          </p>
          <button
            onClick={handleSearch}
            disabled={!partNo}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-100 dark:bg-gray-800 text-xs text-gray-600 dark:text-gray-300">
                    ...
                    <th className="p-2 text-left font-medium">Source Screen</th>
                    <th className="p-2 text-left font-medium">Opening Qty</th>
                    <th className="p-2 text-left font-medium">Received Qty</th>
                    <th className="p-2 text-left font-medium">Dispatch Qty</th>
                    <th className="p-2 text-left font-medium">Closing Qty</th>
                  </tr>
                </thead>
                ...
                <tbody>
                  {currentItems.map((item, i) => (
                    ...
                    >
                      ...
                      <td className="p-2">{item.sourceScreen || '-'}</td>
                      <td className="p-2 text-left">
                        {parseFloat(item.oQty || 0).toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td className="p-2 text-left">
                        {parseFloat(item.rQty || 0).toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td className="p-2 text-left">
                        {parseFloat(item.dQty || 0).toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
                      <td className="p-2 text-left font-medium">
                        {parseFloat(item.cQty || 0).toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
                ...
                <tfoot>
                  <tr className="bg-blue-50 dark:bg-blue-900/20 border-t-2 border-blue-200 dark:border-blue-800">
                    <td colSpan="8" className="p-2 text-right font-bold text-blue-700 dark:text-blue-300">
                      TOTAL:
                    </td>

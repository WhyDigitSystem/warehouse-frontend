...
const SupplierMasterList = ({ onAddNew, onEdit, refreshTrigger }) => {
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      console.log("🔍 [DEBUG] Fetching suppliers with params:", { 
        orgId: ORG_ID,
      ...
      const suppliers = await supplierAPI.getSuppliers(
        ORG_ID, 
        globalParam?.client, 
        globalParam?.branchcode
      );
      
      console.log("🔍 [DEBUG] Actual API result:", suppliers);
      
      const sortedSuppliers = suppliers.sort((a, b) => (b.id || 0) - (a.id || 0));
      setList(sortedSuppliers);
  ...
  const handleExcelDownload = () => {
    ...
    try {
      const excelData = list.map((supplier) => ({
        "Supplier Name": supplier.supplier,
        "Short Name": supplier.supplierShortName,
        "Supplier Type": supplier.supplierType,
        "Contact Person": supplier.contactPerson,
        "Mobile": supplier.mobileNo,
        "Email": supplier.email,
        "Control Branch": supplier.cbranch,
        "Active": supplier.active === "Active" ? "Yes" : "No",
      }));
  ...
  const filtered = list.filter((supplier) =>
    ...
    supplier.supplierShortName?.toLowerCase().includes(search.toLowerCase()) ||
    supplier.contactPerson?.toLowerCase().includes(search.toLowerCase()) ||
    supplier.cbranch?.toLowerCase().includes(search.toLowerCase())
  );
        ...
        >
          <table className="w-full text-sm">
            ...
            <thead>
              <tr className="bg-gray-100 dark:bg-gray-800 text-xs text-gray-600 dark:text-gray-300">
                ...
                <th className="p-2 text-left font-medium">Supplier Name</th>
                <th className="p-2 text-left font-medium">Short Name</th>
                <th className="p-2 text-left font-medium">Supplier Type</th>
            ...
            <tbody>
              {currentItems.map((supplier, i) => (
                ...
                >
                  ...
                  <td className="p-2">{supplier.supplier || '-'}</td>
                  <td className="p-2">{supplier.supplierShortName || '-'}</td>
                  <td className="p-2">{supplier.supplierType || '-'}</td>
                  <td className="p-2">{supplier.contactPerson || '-'}</td>
                  <td className="p-2">{supplier.mobileNo || '-'}</td>
                  <td className="p-2">{supplier.cbranch || '-'}</td>
                  <td className="p-2">
                    <span
          ...
          {filtered.length === 0 && !loading && (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
              {list.length === 0 ? 'No suppliers found' : 'No suppliers match your search'}
            </div>
          )}
        </div>
      )}

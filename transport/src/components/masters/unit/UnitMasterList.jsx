...
const UnitMasterList = ({ onAddNew, onEdit, refreshTrigger }) => {
  ...
  const fetchUnits = async () => {
    console.log("🚀 fetchUnits function called");

    try {
      setLoading(true);

      console.log("🔍 [DEBUG] Fetching units with orgId:", ORG_ID);

      console.log("📡 Making API call...");
      const units = await unitAPI.getUnits(ORG_ID);
      ...
      if (units && Array.isArray(units)) {
        ...
        const sortedUnits = units.sort((a, b) => (b.id || 0) - (a.id || 0));
        setList(sortedUnits);
        console.log(
        ...
        console.warn("⚠️ No units array in response:", units);
        setList([]);
      }
    } catch (err) {
  ...
  useEffect(() => {
    console.log("📈 List state updated:", list.length, "units");
    if (list.length > 0) {
      console.log("📝 First unit in list:", list[0]);
    }
  }, [list]);
  ...
  const handleExcelDownload = () => {
    ...
    try {
      ...
      const fileName = `Units_Export_${new Date()
        .toISOString()
        .slice(0, 10)}.xlsx`;
  ...
  const filtered = list.filter(
    (unit) =>
      unit.uom?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="max-w-6xl mx-auto bg-gray-50 dark:bg-gray-900 p-4">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 flex items-center gap-2 mb-4 shadow-sm">
        ...
        <input
          type="text"
          placeholder="Search by UOM, Unit Name, or Unit Type…"
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="text-sm text-gray-600 dark:text-gray-400">
          Showing {startIndex + 1}-{Math.min(endIndex, filtered.length)} of {filtered.length} units
          {search && ` for "${search}"`}
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <thead>
              <tr className="bg-gray-100 dark:bg-gray-800 text-xs text-gray-600 dark:text-gray-300">
                ...
                <th className="p-2 text-left font-medium">Unit Name</th>
                <th className="p-2 text-left font-medium">Unit Type</th>
              ...
              {currentItems.length > 0 ? (
                currentItems.map((unit, i) => (
                  ...
                  >
                    ...
                    <td className="p-2 font-medium">{unit.uom || "-"}</td>

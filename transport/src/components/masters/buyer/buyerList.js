...
const BuyerMasterList = ({ onAddNew, onEdit, refreshTrigger }) => {
  ...
  const getAPIParams = () => {
    const globalParam = JSON.parse(
      localStorage.getItem("globalParams") || "{}"
    );

    return {
      orgId: ORG_ID,
      client: globalParam?.client || localStorage.getItem("client") || "",
      branchcode:
  ...
  const fetchBuyers = async () => {
    console.log("🚀 fetchBuyers function called");
    ...
    try {
      ...
      console.log("🔍 [DEBUG] Fetching buyers with params:", params);

      // Check if we have required parameters
      if (!params.client || !params.branchcode) {
      ...
      console.log("📊 Buyers received:", buyers);
      console.log("📊 Is array:", Array.isArray(buyers));
      console.log("📊 Length:", buyers?.length);

      if (buyers && Array.isArray(buyers)) {
        console.log(`✅ Processing ${buyers.length} buyers`);
        const sortedBuyers = buyers.sort((a, b) => (b.id || 0) - (a.id || 0));
        setList(sortedBuyers);
        console.log(
          "✅ List state updated with",
          sortedBuyers.length,
          "buyers"
        );
      } else {
        console.warn("⚠️ No buyers array in response:", buyers);
        setList([]);
      }
    } catch (err) {
      ...
      setLoading(false);
      setRefreshing(false);
      console.log("🏁 fetchBuyers completed");
    }
  };

  // Add this useEffect to monitor list state changes
  useEffect(() => {
    console.log("📈 List state updated:", list.length, "buyers");
    if (list.length > 0) {
      console.log("📝 First buyer in list:", list[0]);
    }
  }, [list]);
  ...
  const handleExcelDownload = () => {
    ...
    try {
      const excelData = list.map((buyer) => ({
  ...
  const filtered = list.filter(
    (buyer) =>
      buyer.buyer?.toLowerCase().includes(search.toLowerCase()) ||
      buyer.buyerShortName?.toLowerCase().includes(search.toLowerCase()) ||
      buyer.contactPerson?.toLowerCase().includes(search.toLowerCase()) ||
          ...
          <table className="w-full text-sm">
            ...
            <thead>
              <tr className="bg-gray-100 dark:bg-gray-800 text-xs text-gray-600 dark:text-gray-300">
                ...
                <th className="p-2 text-left font-medium">Buyer Type</th>
                <th className="p-2 text-left font-medium">PAN</th>
                <th className="p-2 text-left font-medium">TAN</th>
                <th className="p-2 text-left font-medium">Contact Person</th>
              ...
              {currentItems.length > 0 ? (
                currentItems.map((buyer, i) => (
                  ...
                  >
                    ...
                    <td className="p-2 font-medium">{buyer.buyer || "-"}</td>
                    <td className="p-2">{buyer.buyerShortName || "-"}</td>
                    <td className="p-2">{buyer.buyerType || "-"}</td>
                    <td className="p-2">{buyer.panNo || "-"}</td>
                    <td className="p-2">{buyer.tanNo || "-"}</td>
                    <td className="p-2">{buyer.contactPerson || "-"}</td>
                    <td className="p-2">{buyer.mobileNo || "-"}</td>
                    <td className="p-2">
                      <span
...
export default BuyerMasterList;

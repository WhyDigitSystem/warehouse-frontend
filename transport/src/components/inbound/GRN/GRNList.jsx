...
const GrnList = ({ onAddNew, onEdit, refreshTrigger }) => {
  ...
  const getAllGrns = async () => {
    try {
      ...
      const response = await grnAPI.getAllGrns(params);
      
      if (response?.paramObjectsMap?.grnVO) {
        setListViewData(response.paramObjectsMap.grnVO);
      } else {
        setListViewData([]);
        addToast("No GRN data found", "info");
      }
    } catch (error) {
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      const response = await grnAPI.getAllGrns({
        branch: loginBranch,
        branchCode: loginBranchCode,
        client: loginClient,
        finYear: loginFinYear,
      ...
      if (response?.paramObjectsMap?.grnVO) {
        const allGrnData = response.paramObjectsMap.grnVO;
      ...
      if (mainRecord.grnDetailsVO && mainRecord.grnDetailsVO.length > 0) {
        mainRecord.grnDetailsVO.forEach((detail) => {
          excelData.push({
            ...
            "Document Date": formatDate(mainRecord.docdate),
            ...
            "Invoice Date": formatDate(detail.invoiceDate),
  ...
  const filteredData = listViewData.filter(
    (item) =>
      ...
      (item.gatePassId && item.gatePassId.toLowerCase().includes(searchTerm.toLowerCase()))
  );
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 gap-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">GRN</h3>
          ...
          <div className="flex flex-1 max-w-md gap-2">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search GRN No, Supplier, or Gate Pass ID..."
                value={searchTerm}
              ...
              {loading ? (
                <tr>
                  <td colSpan="6" className="px-3 py-8 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-500 dark:text-gray-400 mt-2 text-xs">Loading GRN data...</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan="6" className="px-3 py-8 text-center text-gray-500 dark:text-gray-400">
                    {listViewData.length === 0 ? 'No GRN entries found' : 'No matching entries found'}
                  </td>
                </tr>
              ) : (
                filteredData.map((item, index) => (
                  <tr key={`grn-${item.id}-${index}`} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-3 py-2">
                      <div className="flex space-x-1">
                        ...
                        <button
                          onClick={() => {
                            if (window.confirm(`Delete GRN ${item.docId}?`)) {
